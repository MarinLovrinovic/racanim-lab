...
TriangleMesh* ImportMesh(const string& path) {
    ...
    objPath.append("\\f16\\f16.obj"); //za linux pretvoriti u forwardslash
...
int main(int argc, char* argv[]) {
    ...
    controlPolygonObject->material->diffuse = glm::vec3(1, 0, 0);
    ...
    approximatingBezierObject->material->diffuse = glm::vec3(1, 1, 0);
    ...
    bSplineObject->material->diffuse = glm::vec3(0, 0, 1);
    ...
    interpolatingBezierObject->material->diffuse = glm::vec3(0, 1, 1);
    ...
    while (glfwWindowShouldClose(renderer->window) == false) {
        ...
        movingTransform.localX = glm::vec4(glm::normalize(glm::cross(glm::vec3(0, 1, 0), glm::vec3(camera.localZ))), 0);
        movingTransform.localY = glm::vec4(glm::cross(glm::vec3(camera.localZ), glm::vec3(camera.localX)), 0);
